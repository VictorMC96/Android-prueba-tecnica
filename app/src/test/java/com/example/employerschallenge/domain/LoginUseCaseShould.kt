...
import com.example.employerschallenge.fakedata.ANY_INVALID_EMPLOYEE_DETAIL_ID
import com.example.employerschallenge.fakedata.ANY_VALID_EMPLOYEE_DETAIL_ID
...
class LoginUseCaseShould {
    ...
    fun `is true when login is called and userId is valid`() = runTest {
        val result = loginUseCase.login(ANY_VALID_EMPLOYEE_DETAIL_ID).lastOrNull()
    ...
    fun `is false when login is called and userId is invalid`() = runTest {
        val result = loginUseCase.login(ANY_INVALID_EMPLOYEE_DETAIL_ID).lastOrNull()
